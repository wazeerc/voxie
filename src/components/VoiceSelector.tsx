...
const DEFAULT_VOICE_NAME = "Microsoft AvaMultilingual Online (Natural) - English (United States)";

const filterNaturalVoices = (allVoices: SpeechSynthesisVoice[]): SpeechSynthesisVoice[] => {
  const naturalKeywords = ['natural', 'neural', 'premium', 'enhanced', 'wavenet', 'plus'];
...
export const VoiceSelector = memo(({ onVoiceChange }: VoiceSelectorProps) => {
  ...
  const loadVoices = useCallback(() => {
    ...
    if (availableVoices.length > 0) {
      ...
      const voicesToUse = naturalVoices.length > 0 ? naturalVoices : availableVoices;

      setVoices(voicesToUse);

      if (!hasInitializedRef.current) {
        const defaultVoice = voicesToUse.find(
          voice => voice.name.includes(DEFAULT_VOICE_NAME)
        );
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    if (e.key === 'Escape') {
      setIsDropdownOpen(false);
    } else if (e.key === 'ArrowDown' && isDropdownOpen && filteredVoices.length > 0) {
      e.preventDefault();
      const currentIndex = filteredVoices.findIndex(v => v.name === selectedVoice);
      const nextIndex = (currentIndex + 1) % filteredVoices.length;
      handleVoiceSelect(filteredVoices[nextIndex]);
    } else if (e.key === 'ArrowUp' && isDropdownOpen && filteredVoices.length > 0) {
      e.preventDefault();
      const currentIndex = filteredVoices.findIndex(v => v.name === selectedVoice);
      const prevIndex = (currentIndex - 1 + filteredVoices.length) % filteredVoices.length;
  ...
  return (
    <div class="voice-selector" ref={dropdownRef}>
              ...
              {filteredVoices.length > 0 ? (
                filteredVoices.map(voice => (
                  <div
                    ...
                    onKeyDown={(e) => e.key === 'Enter' && handleVoiceSelect(voice)}
                    tabIndex={0}
                    role="option"
                    aria-selected={voice.name === selectedVoice}
                  >
                    {voice.name} ({voice.lang})
                    {voice.localService && <span class="local-indicator"> (Local)</span>}
                  </div>
                ))
              ) : (
      ...
      {voices.length === 0 && (
        <p class="no-voices-message">No voices available. Please try another browser.</p>
      )}
    </div>
  );
});
